...
function searchFiles() {
  ...
  // Não sobrescreve resultado transitório do dashboard quando não há query explícita
  const accessBox = document.getElementById('accessKey');
  const rawGuard = accessBox?.value?.trim() || '';
  if (window.__chartQueryActive && rawGuard === '') {
    // mantém searchResults atual (consulta transitória vinda do dashboard)
    return;
  }
