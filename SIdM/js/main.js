...
async function addNewContent() {
  ...
  const rawHtml = contentBodyEl ? contentBodyEl.innerHTML : '';
...
function toggleDebugMode(force) {
  try {
    ...
    if (enable) {
      window.__debugModeStore.set('html', body.innerHTML);
      const escaped = body.innerHTML.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      ...
      const saved = window.__debugModeStore.get('html');
      if (typeof saved === 'string') body.innerHTML = saved;
      body.removeAttribute('data-debug-mode');
      window.__debugMode = false;
    }
    const btn = document.getElementById('debug-toggle-btn');
    if (btn) { btn.classList.toggle('active', !!window.__debugMode); btn.textContent = window.__debugMode ? 'Exibição' : 'HTML'; }
